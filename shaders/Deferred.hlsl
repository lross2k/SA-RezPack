...
float4 SunLightingPS(PS_QUAD_IN i) : SV_Target
{
	...
	float Roughness = 1.1 - Parameters.y;
...
float4 FinalPassPS(PS_QUAD_IN i) : SV_Target
{
	...
	else
	{
        ...
        float3 DiffuseTerm = (Lighting.xyz + vSkyLightCol.rgb * 1.0f * AmbientOcclusion);
		...
		switch (MaterialType) /* As seen in TBP shaders */
        {
        // Cars
        case 1:
            DiffuseTerm	= Lighting.xyz + 0.6f * AmbientOcclusion * AlbedoColor.rgb;
            FresnelCoeff = lerp(0.0f, FresnelCoeff, Parameters.y);
            ReflectionTerm *= 6.0f;
            break;
        // Skins
        case 2:
            DiffuseTerm	= Lighting.xyz + 0.4f * AmbientOcclusion * AmbientOcclusion;
			ReflectionTerm = 0.0f;
			FresnelCoeff = 0.0f;
            break;
        // Other stuff, I couln't find anything else that was special
        default:
            ReflectionTerm *= 0.5f;
            break;
        }
